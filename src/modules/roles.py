...
class RoleCommands(commands.Cog, name=MODULE_NAME):
    def __init__(self, bot: ThisIsTheBot):
        self.bot = bot

    @commands.command()
    async def roles(self, ctx: commands.Context[ThisIsTheBot]) -> None:
        '''Send a friendly hello message'''
        await self.bot.send(ctx, f'Hello {ctx.message.author.roles} !')
